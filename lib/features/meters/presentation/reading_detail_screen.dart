...
class _ReadingDetailScreenState extends ConsumerState<ReadingDetailScreen> {
  ...
  Widget _buildContent(MeterReading reading) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Projektstand')),
      body: ListView(
        ...
        children: [
          ...
          Text(
            'Gespeicherte Projektprotokolle',
            style: Theme.of(
              context,
            ).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.w800),
          ),
          ...
          if (singleExports.isNotEmpty) ...[
            ...
            for (final export in singleExports)
              EvidenceExportCard(
                ...
                detail:
                    '${currentProgressLabel(reading.meter.unit)}: ${reading.value.displayText}\n${export.photoMode.labelFor(export.kind)}',
              ...
              children: [
                ...
                const Expanded(
                  child: Column(
                    ...
                    children: [
                      Text(
                        'Projektprotokoll dieses Projektstands',
    ...
    return Column(
      ...
      children: [
        ...
        OutlinedButton.icon(
          ...
          label: const Text('Projektstand löschen'),
        ),
      ],
    );
  }
...
class _RevisionList extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if (revisions.isEmpty) {
      return const Text(
        'Für diesen Projektstand gibt es noch keine Korrekturen.',
      );
    }
...
class _RevisionEntry extends StatelessWidget {
  ...
  String _displayValue(String key, String value) {
    ...
    if (key == 'Zeitpunkt des Projektstands') {
    ...
    if (key == 'Aktuelle Reihe' || key == 'Aktuelle Runde') {
      return '$value ${reading.meter.unit}';
    }
    return value;
  }
}
