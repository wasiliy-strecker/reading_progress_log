...
import 'package:strick_haekelbuch/core/integrity/integrity_copy.dart';
import 'package:strick_haekelbuch/core/utils/formatters.dart';
...
void main() {
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      expect(
        find.text('Für diesen Projektstand gibt es noch keine Korrekturen.'),
        findsOneWidget,
      );
      ...
      await tester.scrollUntilVisible(
        find.text('Projektprotokoll dieses Projektstands'),
        250,
        scrollable: scrollable,
      );
  ...
  testWidgets('shows correction reason and newest changes first', (
    ...
    final readings = MemoryReadingRepository()
      ...
      ..revisions[reading.id] = [
        ReadingRevision(
          ...
          changes: const {
            'Aktuelle Reihe': ReadingChange(before: '24,1', after: '42,1'),
          },
        ),
        ReadingRevision(
          id: 'revision_newer',
    ...
    expect(find.text('24,1 Runden'), findsOneWidget);
    expect(find.text('42,1 Runden'), findsWidgets);
    expect(find.text('Projektstandfoto geändert'), findsOneWidget);
    expect(find.text('OCR-Kandidat'), findsNothing);
  ...
  testWidgets('single PDF action immediately shows indeterminate progress', (
    ...
    final scrollable = find
        ...
        .first;
    await tester.scrollUntilVisible(
      find.text('Projektprotokoll als PDF erstellen'),
      250,
      scrollable: scrollable,
    );
    expect(find.text('Projektprotokoll dieses Projektstands'), findsOneWidget);
    expect(find.text(pdfPurposeText), findsOneWidget);

    await tester.tap(find.text('Projektprotokoll als PDF erstellen'));
    await tester.pumpAndSettle();
    ...
    expect(find.text('Projektprotokoll als PDF erstellen'), findsOneWidget);
  });
  ...
  testWidgets('shows saved single evidence and keeps creation available', (
    ...
    final createButton = tester.widget<FilledButton>(
      find.widgetWithText(FilledButton, 'Projektprotokoll als PDF erstellen'),
    );
    ...
    expect(
      tester
          .getTopLeft(
            find.widgetWithText(
              FilledButton,
              'Projektprotokoll als PDF erstellen',
            ),
          )
          .dy,
      lessThan(
    ...
    expect(find.text('Aktueller Projektprotokoll'), findsNothing);
    expect(find.byKey(const ValueKey('current-evidence-status')), findsNothing);

    await tester.ensureVisible(find.text('Projektprotokoll als PDF erstellen'));
    await tester.tap(find.text('Projektprotokoll als PDF erstellen'));
    await tester.pumpAndSettle();
    expect(
      tester
  ...
  testWidgets('keeps a single PDF visible when deletion fails', (tester) async {
    ...
    expect(
      find.text(
        'Projektprotokoll konnte nicht gelöscht werden. Bitte versuche es erneut.',
      ),
      findsOneWidget,
    );
  });
  ...
  testWidgets('missing single evidence file does not block recreation', (
    ...
    exports.items['single_missing'] = EvidenceExportRecord(
      ...
      filePath: '/tmp/definitely-missing-strick-haekelbuch.pdf',
      pdfSha256: 'c' * 64,
      manifestSha256: unchangedReadingManifest,
    );
  ...
  testWidgets('a correction enables a new single evidence PDF', (tester) async {
    ...
    readings.revisions[reading.id] = [
      ReadingRevision(
        ...
        reason: 'Aktuelle Reihe korrigiert',
        changes: const {
          'Aktuelle Reihe': ReadingChange(before: '41,2', after: '42,1'),
        },
      ),
    ];
