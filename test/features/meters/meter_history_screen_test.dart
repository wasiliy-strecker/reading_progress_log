...
import 'package:strick_haekelbuch/app/app_router.dart';
...
void main() {
  for (final count in [0, 1, 6, 9, 10, 11, 21]) {
    testWidgets(
      ...
      (tester) async {
        ...
        if (count > 1) {
          expect(
            find.text('${count < 10 ? count : 10} von $count Projektständen'),
            findsOneWidget,
          );
        }
        expect(
        ...
        if (count == 0) {
          expect(
            find.text('Noch keine Projektstände vorhanden.'),
            findsOneWidget,
          );
        }
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      expect(
        find.text('Kein passender Projektstand gefunden.'),
        findsOneWidget,
      );
      expect(tester.takeException(), isNull);
    },
