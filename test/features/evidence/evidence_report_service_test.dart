...
import 'package:strick_haekelbuch/features/meters/domain/meter_reading_order.dart';
...
void main() {
  ...
  test(
    ...
    () {
      ...
      expect(rows.first[1], '125,5 Reihen');
      expect(rows.first[2], '25,5 Reihen');
      expect(rows.last[2], '–');
  ...
  test('exports a complete year of daily readings without photos', () async {
    ...
    if (const bool.fromEnvironment('PDF_TEXT_AUDIT')) {
      ...
      for (final page in pages) {
        ...
        expect(page, contains('Projektstand'));
        expect(page, contains('Fortschritt'));
      }
    }
  });
  ...
  test('allows repeated history PDFs and uses current meter data', () async {
    ...
    final first = await service.createHistory(
      meter: _meter(location: 'Alter Nadelstärke'),
      readings: [reading],
      revisions: {reading.id: const []},
      photoMode: EvidencePhotoMode.withoutPhotos,
    );
    final second = await service.createHistory(
      meter: _meter(label: 'Strom Neu', location: 'Neuer Nadelstärke'),
      readings: [reading],
      revisions: {reading.id: const []},
      photoMode: EvidencePhotoMode.withoutPhotos,
    );
    ...
    final report = await service.createSingle(
      ...
      revisions: [
        ReadingRevision(
          ...
          changes: {
            'Aktuelle Reihe': const ReadingChange(
              before: '00132,4',
              after: '00123,4',
            ),
            'Prüfwert des Fotos (SHA-256)': ReadingChange(
