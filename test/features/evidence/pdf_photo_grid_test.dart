      ...
      test(
        ...
        () async {
          ...
          for (var i = 0; i < count; i++) {
            ...
            if (i == 0) {
              final heading = captions['Aktuelle Reihe']!;
          ...
          if (!history && count == 3) {
            ...
            await File(
              report.record.filePath,
            ).copy('${verification.path}/strick_haekelbuch-grid-single-3.pdf');
            await File(
              '${verification.path}/strick_haekelbuch-grid-single-3.xml',
            ).writeAsString(result.stdout as String);
            expect(
              pages,
              hasLength(1),
              reason:
                  'Three photos and a short project header should fit on one page',
            );
          }
        },
      );
