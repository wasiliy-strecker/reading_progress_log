...
import 'package:strick_haekelbuch/features/evidence/presentation/saved_history_pdfs.dart';
import 'package:strick_haekelbuch/features/meters/data/in_memory_meter_repositories.dart';
...
void main() {
  for (final count in [6, 10, 11, 12]) {
    testWidgets('$count PDFs: paginate only above ten records', (tester) async {
      ...
      expect(find.text('$count Projektprotokolle'), findsOneWidget);
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      expect(find.text('100 Projektprotokolle'), findsOneWidget);
      ...
      await tester.pumpAndSettle();
      expect(find.text('101 Projektprotokolle'), findsOneWidget);
      expect(find.byType(EvidenceExportCard), findsNothing);
  ...
  testWidgets('page error and file error are retryable on narrow display', (
    ...
    await _tap(tester, _expansion);
    expect(
      find.text('Projektprotokolle konnten nicht geladen werden.'),
      findsOneWidget,
    );
...
Future<void> _tap(WidgetTester tester, Key key) async {
  final finder = key == _expansion
      ? find.text('Gespeicherte Projektprotokolle')
      : find.byKey(key);
