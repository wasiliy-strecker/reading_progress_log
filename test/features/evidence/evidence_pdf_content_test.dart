...
void main() {
  ...
  test('historical fields show only differences and preserve empty values', () {
    ...
    final current = MeterSnapshot.fromMeter(
      _meter().copyWith(
        ...
        unit: 'Runden',
        meterNumber: 'NEU-999',
        location: 'Neuer Nadelstärke',
      ),
    );
    expect(EvidenceReportService.historicalMeterData(original, current), [
      ['Technik', _meter().type.label],
      ['Projektname', 'Alter Projektname'],
      ['Garn', 'ALT-777'],
      ['Nadelstärke', 'Nicht angegeben'],
      ['Zählweise', 'Reihen'],
    ]);
  });
  ...
  test(
    'page progress uses the older reading and retains Strick & Häkelbuch wording',
    () async {
      final temp = await Directory.systemTemp.createTemp(
        'strick_haekelbuch_progress_',
      );
      ...
      expect(rows.map((row) => row[2]), ['45 Reihen', '–']);
      final report =
      ...
      if (_auditPdfText) {
        ...
        ).copy('${auditDirectory.path}/strick_haekelbuch-progress.pdf');
        ...
        expect(text, contains('Reihe 85 130 = 45 Reihen Fortschritt'));
        expect(text, contains('Projektprotokoll'));
        expect(text, contains('STRICK & HÄKELBUCH'));
        expect(text, isNot(contains('LESELOG')));
        expect(text, contains('Projektverlauf'));
        ...
        test(
          ...
          () async {
            ...
            if (_auditPdfText) {
              ...
              expect(text, contains('Projektangaben bei Erfassung'));
              ...
              if (kind == EvidenceExportKind.meterHistory) {
                expect(text, contains('Aktuelle Projektangaben'));
              ...
              await File(report.record.filePath).copy(
                '${audit.path}/strick_haekelbuch-${kind.name}-${mode.name}-${longText ? 'long' : 'short'}.pdf',
              );
            }
          },
        );
...
ReadingRevision _revision({bool longText = false}) => ReadingRevision(
  ...
  changes: {
    'Aktuelle Reihe': const ReadingChange(before: '95,0', after: '100,0'),
    'Notiz': ReadingChange(
