...
const _singleLabel = 'Projektprotokoll als PDF erstellen';
const _historyLabel = 'Projektprotokoll für den Projektverlauf erstellen';
...
void main() {
  for (final hasPhotos in [false, true]) {
    testWidgets('single PDF uses only its own photos: $hasPhotos', (
      ...
      if (!hasPhotos) {
        ...
        expect(find.text('Keine aktuellen Fotos vorhanden.'), findsOneWidget);
  ...
  for (final single in [false, true]) {
    for (final hasCurrent in [false, true]) {
      testWidgets(
        'project current photo list wins over archived photos: single=$single current=$hasCurrent',
        (tester) async {
          final readings = _Readings();
          final photoReading = _reading(hasPhotos: true);
          final first = photoReading.currentPhotoVersion!;
          final second = _reading(
            id: 'second',
            hasPhotos: true,
          ).currentPhotoVersion!;
          readings.items['reading'] = photoReading.copyWith(
            photos: hasCurrent ? [first, second] : const [],
            photoHistory: hasCurrent ? const [] : [first, second],
          );
          await _open(tester, readings, _Reports(), single: single);
          await _pressPdf(tester, single: single);
          expect(tester.widget<ListTile>(_photoOption).enabled, hasCurrent);
          expect(
            find.text('Keine aktuellen Fotos vorhanden.'),
            hasCurrent ? findsNothing : findsOneWidget,
          );
          await _cancelSheet(tester);
          expect(tester.takeException(), isNull);
        },
