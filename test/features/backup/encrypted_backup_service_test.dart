...
void main() {
  ...
  test('accepts six-character passwords and rejects shorter ones', () async {
    ...
    expect(
      await File(latestBackup.path).parent
          .list()
          .where((entity) => entity.path.endsWith('.shbackup'))
          .length,
      1,
    );
  });
  ...
  test('encrypted backup round-trips domain data, photos and PDFs', () async {
    ...
    final meter = _meter().copyWith(
      ...
      // Pre-sync Strick & Häkelbuch schedules have no hourly start timestamp.
    ...
    final revision = ReadingRevision(
      ...
      changes: const {
        ...
        'Aktuelle Reihe': ReadingChange(before: '40', after: '42,5'),
        'Zeitpunkt des Projektstands': ReadingChange(
    ...
    final legacyPayload = <String, dynamic>{
      'manifest': {
        'format': 'strick_haekelbuch_backup',
        'schemaVersion': 2,
