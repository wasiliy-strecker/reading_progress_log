...
import 'package:strick_haekelbuch/features/backup/application/binary_backup_codec.dart';
import 'package:strick_haekelbuch/features/backup/application/encrypted_backup_service.dart';
...
void main() {
  ...
  test(
    'v4 restores and repairs every current and historical photo with revision IDs',
    () async {
  ...
  test(
    'pre-change v3 backup decrypts using its original authenticated version',
    () async {
      ...
      const path = 'test/fixtures/legacy_single_photo_v3.shbackup';
      expect((await service.inspect(path, 'fixture-only')).readingCount, 1);
      expect((await service.restore(path, 'fixture-only')).readings, 1);
      final reading = readings.items.values.single;
      expect(reading.photos, null);
      expect(reading.currentPhotos, hasLength(1));
      expect(await File(reading.currentPhotos.single.path).readAsBytes(), [
        1,
        2,
        3,
        4,
      ...
      expect(
        await const IntegrityService().readingManifestHash(reading),
        reading.manifestSha256,
      );
    },
  );
