...
import 'package:strick_haekelbuch/features/backup/presentation/settings_screen.dart';
...
void main() {
  ...
  testWidgets('valid backup selection opens the password dialog', (
    ...
    final picker = _FakeBackupFilePicker(['/tmp/import.shbackup']);
    await tester.pumpWidget(
      ProviderScope(
  ...
  testWidgets(
    ...
    (tester) async {
      final exporter = _FakeBackupFileExporter([
        const BackupSaveResult.saved('gesichertes-backup.shbackup'),
      ]);
      ...
      expect(
        find.textContaining('gesichertes-backup.shbackup'),
        findsOneWidget,
      );
  ...
  testWidgets('cancelled save can choose a destination again', (tester) async {
    final exporter = _FakeBackupFileExporter([
      ...
      const BackupSaveResult.saved('zweiter-versuch.shbackup'),
    ]);
    final service = _ControlledBackupService();
    await tester.pumpWidget(
      ProviderScope(
    ...
    expect(find.text('Backup gespeichert'), findsOneWidget);
    expect(find.textContaining('zweiter-versuch.shbackup'), findsOneWidget);
    expect(tester.takeException(), isNull);
  });
  ...
  testWidgets('failed save can be retried without recreating the backup', (
    ...
    final exporter = _FakeBackupFileExporter([
      ...
      const BackupSaveResult.saved('nach-fehler.shbackup'),
    ]);
    final service = _ControlledBackupService();
    await tester.pumpWidget(
      ProviderScope(
